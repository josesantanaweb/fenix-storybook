import { getHandleSelector, getRectSelector, getSelector, moveBoundary, moveRect } from '../utils'
import { ChartPrices } from '..'

export const onDragBuyHandler = ({ y, y2, marketPriceY }: { y: number; y2: number; marketPriceY: number }) => {
  const yOpposite = Number(getSelector(getHandleSelector('sell', 'line1')).select('line').attr('y1'))
...
export const onDragSellHandler = ({ y, y2, marketPriceY }: { y: number; y2: number; marketPriceY: number }) => {
  const yOpposite = Number(getSelector(getHandleSelector('buy', 'line1')).select('line').attr('y1'))
  // SellMin is dragged below BuyMin
  if (y > yOpposite) {
    return
  }
...
export const onDragRectHandler = ({ yPos, marketPriceY }: { yPos: ChartPrices<number>; marketPriceY: number }) => {
...
export const handleStateChange = ({ yPos, marketPriceY }: { yPos: ChartPrices<number>; marketPriceY: number }) => {
