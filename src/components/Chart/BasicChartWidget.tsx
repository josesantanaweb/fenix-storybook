...
export default function BasicChartWidget({ width, height }: { width: number; height: number }) {
  const [chartReady, setChartReady] = useState(false)
  const fenixChart = useRef<HTMLElement>(null)
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const chart = useRef<any>(null)

  useEffect(() => {
    if (!fenixChart.current) return

    fenixChart.current.innerHTML = ''

    chart.current = createChart(fenixChart.current, {
      height: height - 1,
      width: width,
      layout: {
        background: {
    ...
    const candleSeries = chart.current.addCandlestickSeries({
      upColor: '#FF922E',
      downColor: '#A13114',
      borderDownColor: '#A13114',
      borderUpColor: '#FF922E',
      wickDownColor: '#A13114',
      wickUpColor: '#FF922E',
    })
  ...
  useEffect(() => {
    chart.current.resize(width, height > 854 ? 854 : height - 1)
  }, [width, height])
  ...
  return (
    <>
      ...
      <div
        ref={fenixChart as React.RefObject<HTMLDivElement>}
        id="fenix_graph"
        className={chartReady ? 'max-h-[845px]' : 'hidden'}
      />
    </>
  )
}
